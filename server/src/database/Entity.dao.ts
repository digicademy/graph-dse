...
export default class EntityDAO {
  public static async getEntities(type?: string): Promise<IEntity[]> {
    const query: string = `
    MATCH (e:Entity)<-[:REFERS_TO]-(:Spo)--(:Text)<-[:HAS_TEXT]-(m:Metadata) 
    ${type ? "WHERE e.type = $type" : ""} 
    RETURN properties(e) as entity, collect(DISTINCT properties(m)) as metadata`;
  ...
  public static async getOccurrences(entityId: string): Promise<IMetadata[]> {
    const query: string = `
    MATCH (e:Entity {guid: $entityId})<-[:REFERS_TO]-(:Spo)--(:Text)<-[:HAS_TEXT]-(m:Metadata)
    RETURN collect(DISTINCT properties(m)) as occurrences`;
